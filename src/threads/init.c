...
pintos_init (void)
{
  ...
  while (1){
    ...
    printf("CS2043> "); // tokens

    // combining characters to get complete command
    while (true) {
      char c = input_getc(); // getting character c via buffer specified in input.c file

      // terminate the loop responsible for getting char one by one.
      if (c == '\r') {
        printf("\n");
        command[cmd_index] = '\0'; // adding null char to the command word
        break;
      } 
      else {
        putchar(c);  // Print characters as they are typed in the screen
        command[cmd_index++] = c;
      }
    }
    ...
    if (!strcmp(command,"whoami")){
      printf("CS2043 - 210491P  Pranavan\n");
    }
    // case: shutdown
    else if (!strcmp(command,"shutdown")){
      printf("Shutting Down!");
      shutdown_power_off(); // shutdown and closes qemu window.
    }

    // case: time
    else if (!strcmp(command,"time")){
      printf("CS2043 - %lu sec.\n", rtc_get_time()); // rtc_get_time() from src/devices/rtc.c
    }

    // case: ram
    else if (!strcmp(command,"ram")){
      printf("CS2043 - %d bytes available.\n", init_ram_pages * PGSIZE);
    }

    // case: thread
    else if (!strcmp(command,"thread")){
      printf("CS2043 - ");
      thread_print_stats(); // thread_print_stats() from threads.c
    }

    // case: priority
    else if (!strcmp(command,"priority")){
      printf("CS2043 - priority: %d\n", thread_get_priority()); // thread_get_priority() from threads.c
    }
...
// exit message (from interactive shell)
