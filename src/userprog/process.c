...
start_process (void *file_name_)
{
  ...
  success = load (f_name, &if_.eip, &if_.esp, &save_ptr);
  struct thread* t = thread_current();
  if (success){
    t->cp->load_status = LOADED; // loaded successfully
  }else {
    t->cp->load_status = LOAD_FAIL; // loading failed
  }
...
process_wait (tid_t child_tid UNUSED) {
  struct child_process* c_ptr = find_child_process(child_tid);
  if (!c_ptr){
    return ERROR; // child not found
  }
  if (c_ptr->wait){
    return ERROR; // already waiting child process
  }
  c_ptr->wait = 1; 
  while (!c_ptr->exit){
    asm volatile ("" : : : "memory");
  }
  int status = c_ptr->status;
  remove_child_process(c_ptr);
  return status;
}
...
process_exit (void)
{
  ...
  lock_acquire(&filesys_lock); // get the lock of file_system (resource)
  process_close_file(CLOSE_ALL_FD);
  /* check if current thread is an executable if so we will close it */
  if (cur->executable){
    file_close(cur->executable);
  }
  lock_release(&filesys_lock); // return lock
  remove_all_child_processes();
  
  // check the parent of the thread is alive (zombie)
...
setup_stack (void **esp, char **saveptr, const char *filename) 
{
  ...
  if (kpage != NULL){
    ...
    *esp = PHYS_BASE; // effective stack pointer
  } else {
    palloc_free_page(kpage);
    return success;
  }
  ...
  // copying command line into cont and resizing to necessary size
  token = (char*)filename;
  while (token != NULL) {
    cont[argc] = token;
    argc++;
  ...
  // copying content of cont over to argv
  for (i = argc-1; i >= 0; i--){
    *esp -= strlen(cont[i])+1;
  ...
  // word alignment by word size (4 bytes)
  i = (size_t) *esp % 4;
  if (i){
    *esp -= i;
    byte_size += i;
  ...
  *esp -= sizeof (char**); // updating effective stack pointer
  byte_size += sizeof (char**);
