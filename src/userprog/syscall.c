...
syscall_open(const char *file_name){
  struct file *f_ptr;
  lock_acquire(&filesys_lock);
  f_ptr = filesys_open(file_name); // from filesys.h
  if (!f_ptr){
    lock_release(&filesys_lock);
    return ERROR;
  }
...
syscall_filesize(int filedes){
  struct file *f_ptr;
  lock_acquire(&filesys_lock);
  f_ptr = filesys_open(filedes); // from filesys.h
  if (!f_ptr){
    lock_release(&filesys_lock);
    return ERROR;
  }
  int filesize = file_length(f_ptr); // from file.h
  lock_release(&filesys_lock);
  return filesize;
}
...
syscall_read(int filedes, void *buffer, unsigned length){
  ...
  int bytes_read = file_read(f_ptr, buffer, length); // from file.h
  lock_release (&filesys_lock);
  return bytes_read;
}
...
syscall_write (int filedes, const void * buffer, unsigned byte_size){
    ...
    int bytes_count = file_write(f_ptr, buffer, byte_size); // file.h
    lock_release (&filesys_lock);
    return bytes_count;
}
...
syscall_tell(int filedes){
  ...
  off_t offset = file_tell(f_ptr); //from file.h
  lock_release(&filesys_lock);
  return offset;
}
...
validate_buffer(const void* buf, unsigned byte_size){
  ...
  local_buffer = (const void*)buf;
  while (itr < byte_size) {
    if (local_buffer < USER_VIRTUAL_ADDRESS_LIMIT || !is_user_vaddr(local_buffer)){
      syscall_exit(ERROR);
    }
    local_buffer++;
    itr++;
